...
function readiness(a,confluence,dataScore,gate){
  const side=sideOf(a);
  ...
  if(a?.status==="READY"&&confluence>=72)return {state:"READY",action:side,reason:"Directional and evidence thresholds are satisfied."};
  if(confluence>=55)return {state:"WATCH",action:side,reason:"The setup is developing; confirmation is incomplete."};
  return {state:"NO_TRADE",action:"WAIT",reason:"Evidence is not aligned enough for a directional setup."};
}
function evaluate(x={}){
  ...
  const r=readiness(a,qScore,data.score,x.propGate),lv=levels(a);
