...
function liteCopilot(mode,market,trade,question){
  const m=market||{},call=String(m.status==="READY"?m.type:m.status==="WATCH"?"WATCH":m.status==="WAITING"?"NO TRADE":m.type||"NO TRADE"),regime=String(m.regime||"UNKNOWN"),score=Number(m.score||0),rsi=Number.isFinite(Number(m.rsi))?Number(m.rsi).toFixed(2):+§uçâçT",adx=Number.isFinite(Number(m.adx))?Number(m.adx).toFixed(2):"ºw^~)Þt",structure=String(m.structure||"éÝyø§yÔ"),mtf=m.mtf?("4H "+(m.mtf.higher||"UNKNOWN")+" · 15M "+(m.mtf.lower||"UNKNOWN")):"",deriv=m.derivatives||{};
  if(mode==="trade"){
